	...
	function deleteNews($aryNewsId) {
		...
		if(is_array($aryNewsId) && sizeof($aryNewsId) > 0) {
			$strNewsId = implode("','", $aryNewsId);
				$sql = "DELETE FROM ".$this->prefix."news WHERE news_id IN ('$strNewsId')";
				$affectedRows = $this->db->query($sql);	
				
				$sql = "DELETE FROM ".$this->prefix."news_des WHERE news_id IN ('$strNewsId')";
				$affectedRows = $this->db->query($sql);	
		}
	...
	function createNews($aryNewsInfo) {
		...
		$sql = "INSERT INTO ".$this->prefix."news(
					...
					VALUES(
							...
							'" . strtotime($aryNewsInfo['epl_date_start']) . "', 
							'" . strtotime($aryNewsInfo['epl_date_expired']) . "', 
							'" . $aryNewsInfo['epl_position'] . "', 
							'" . $aryNewsInfo['history_year'] . "', 
...
//		$lang_id = 0;
		...
		$sql = "SELECT n.*, nd.* FROM ".$this->prefix."news n LEFT JOIN ".$this->prefix."news_des nd 
				 ON(n.news_id = nd.news_id AND nd.lang_id = '$lang_id') WHERE n.news_id = " . $id." ";
...
//			}
	...
	function updateNews($aryNewsInfo) {
		...
		$sql = "UPDATE ".$this->prefix."news SET news_image = '" . $aryNewsInfo['news_image'] . "', 
						...
						img_thumb = '" . $aryNewsInfo['img_thumb'] . "', 
						...
						epl_date_start = '" . strtotime($aryNewsInfo['epl_date_start']) . "', 
						epl_date_expired = '" . strtotime($aryNewsInfo['epl_date_expired']) . "', 
						epl_position = '" . $aryNewsInfo['epl_position'] . "', 
						history_year = '" . $aryNewsInfo['history_year'] . "', 
						update_date = ". time() . ",   
