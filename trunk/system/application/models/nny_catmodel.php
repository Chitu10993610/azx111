<?
class Nny_catModel extends Nny_Model {
/**
 * MODULE NAME   : catmodel.php
 *
 * DESCRIPTION   : Nny_cat model controller
 *
 * MODIFICATION HISTORY
 *   V1.0   2009-03-20 01:12 AM   - Pradesh Chanderpaul     - Created
 *
 * @package             cat
 * @subpackage          Nny_cat model component Class
...
//   }
   ...
   function find($filters = NULL, $start = NULL, $count = NULL) {
      ...
      // ///////////////////////////////////////////////////////////////////////
//      $this->table_record_count = $this->db->count_all( 'cat' );
		$query = $this->db->query('SELECT COUNT(*) as total_row FROM '.$this->prefix.'cat ' . $filters);
		$row = $query->row();
      ...
      $sql = 'SELECT * FROM '.$this->prefix.'cat ' . $where_clause . $limit_clause;
   ...
   function retrieve_by_pkey($idField) {
      ...
      $query = $this->db->query("SELECT * FROM ".$this->prefix."cat WHERE id = '$idField' LIMIT 1");
   ...
   function add( $data ) {
      ...
      $sql = $this->db->insert_string('cat', $data);
   ...
   function modify($keyvalue, $data) {
      ...
      // Build up the SQL query string
      $where = "id = $keyvalue";
      $sql = $this->db->update_string('cat', $data, $where);
   ...
   function delete_by_pkey($idField, $site_id)
   {
      // Load  the db library
      $this->load->database();
      $query = $this->db->query("DELETE FROM ".$this->prefix."cat WHERE id = '$idField' AND site_id = $site_id");
	...
	function set_News_url($news_url) {
	   ...
	   * @return $array
	 */
	function getCategoryTree(&$aryResult, $parent_id = 0, $level = 0, $ignore_cat = array(), $filter = '') {
//		$lang_id = $this->session->userdata('lang_id');
		$lang_id = 'vi';
		
		//$str_ignore_condition = " and c.cat_id <> '$ignore_cat'";
		$sql = "SELECT c.cat_id, cat_name, c.parent_id,  c.cat_order, c.cat_status FROM ".$this->prefix."cat c, ".$this->prefix."cat_des cd WHERE c.cat_id = cd.cat_id AND parent_id = " .(int)$parent_id . " AND lang_id = '" . $lang_id . "' ORDER BY c.cat_order, cd.cat_name";
		//die($sql);
		$level++;
		...
		foreach ($rs->result_array() as $ary) {
			if (!in_array($ary['cat_id'], $ignore_cat)) {
				...
				$this->getCategoryTree($aryResult, $ary['cat_id'], $level, $ignore_cat, $filter);
			}	
		}
		$level--;
		//print_r($aryResult);							 									 
	...
	function getTotalCatByParentId($parent_id = 0) {
		...
		$sql = "SELECT COUNT(c.cat_id) AS totalCat FROM ".$this->prefix."cat c, ".$this->prefix."cat_des cd WHERE c.cat_id = cd.cat_id AND  parent_id = " .(int)$parent_id . " AND lang_id = '$lang_id'";
		$rs = $this->db->query($sql);
	...
	function createCat($aryCatInfo) {
		...
		$sql = "INSERT INTO ".$this->prefix."cat(
		create_date, 
		parent_id, 
		cat_image, 
		cat_status
		...
		VALUES(
		NOW(), 
		" . $aryCatInfo['parent_id'] . ", 
		'" . $aryCatInfo['cat_image'] . "', 
		" .  $aryCatInfo['cat_status'] . 
		")";
		$this->db->query($sql);
...
//		foreach ($aryLanguageList as $aryLanguage) {
			...
			$sql = "INSERT INTO ".$this->prefix."cat_des (cat_id, cat_name, lang_id) VALUES(" .$insert_id. ",'" .$cat_name. "','" .$lang_id. "')";
			$this->db->query($sql);	
//		}
	...
	function editCat($aryCatInfo) {
		...
		$sql = "UPDATE ".$this->prefix."cat SET last_modefied = NOW(), 
							parent_id = " . $aryCatInfo['parent_id'] . ", 
							cat_image = '" . $aryCatInfo['cat_image'] . "', 
							cat_status = " .  $aryCatInfo['cat_status'] ."  WHERE cat_id =".$cat_id;
				
		$this->db->query($sql);
		$cat_name = $aryCatInfo['cat_name'];
		$sql = "UPDATE ".$this->prefix."cat_des SET cat_name = '" .$cat_name. "' WHERE cat_id = " .$cat_id. " AND lang_id = '".$aryCatInfo['lang_id']."'";
	...
	/**
	    * function trans. translate category 
	    * @access public
	    * @param array $aryCatInfo
	    * @param array $aryLanguageList
	    * @author Bui Thanh Dung
	    * @return infectedNumber
	 */
	function trans($aryCatInfo, $traslated = 'true') {
		$affectedRows = 0;
		$cat_name = $this->db_input($aryCatInfo['cat_name']);
				
		if($traslated) {
			$sql = "UPDATE ".$this->prefix."cat_des SET cat_name = '" .$cat_name. "' WHERE cat_id = " .$aryCatInfo['cat_id']. " AND lang_id = '".$aryCatInfo['lang_id']."'";
		}
		else {
			$sql = "INSERT INTO ".$this->prefix."cat_des (cat_id, cat_name, lang_id) VALUES(" .$aryCatInfo['cat_id']. ",'" .$cat_name. "','" .$aryCatInfo['lang_id']. "')";
		}
	...
	/**
	    ...
	    * @return $array
	 */
	function getCatInfoBytId($catId, $lang_id = 'vi') {

		$aryCatInfo = array();
		if(!is_numeric($catId)) return $aryCatInfo;
		
		$sql = "SELECT c.*, cd.* FROM ".$this->prefix."cat c, ".$this->prefix."cat_des cd WHERE c.cat_id = cd.cat_id AND cd.lang_id = '$lang_id' AND cd.cat_id = " . $catId;
		$rs = $this->db->query($sql);
	...
	function deleteCat($catId) {
		...
		$sql = "DELETE cd FROM ".$this->prefix."cat c, ".$this->prefix."cat_des cd WHERE cd.cat_id IN (SELECT cat_id FROM ".$this->prefix."cat WHERE parent_id = " . $catId . ")";
		$this->db->query($sql);
		
		//delete cat and subcat in news_type
		$sql = "DELETE c, cd FROM ".$this->prefix."cat c, ".$this->prefix."cat_des cd WHERE (c.cat_id = " . $catId . " OR parent_id =" . $catId . ") AND cd.cat_id =" . $catId ;
		$this->db->query($sql);	
	...
	function sortCat($aryCatOrder, $aryCatId) {
		...
		foreach ($aryCatOrder as $key => $catOrder) {		
			...
			$sql = "UPDATE ".$this->prefix."cat SET cat_order = " .  $catOrder ."  WHERE cat_id =".$cat_id;
			$affectedRows = $this->db->query($sql);
		}
