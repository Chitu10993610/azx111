...
| 
| Last changed:
| 29 Nov '07 Thomas Traub tomcode.com 
| 	constructor: added patch for using CI session
|	check : added rememberMe check to lougout of expired connection
...
class Userauth {
	...
	function Userauth()
	{
		...
		// in case CI session class is used the cookie needs to live longer then the login lasts
		// this is a quick and dirty debugging for PHP5 where native session does not work
		if(class_exists ('MY_Session'))
		{
			$sess_expiration = $this->obj->config->item('login_expiration') +10;
			$this->obj->config->set_item('sess_expiration', $sess_expiration);
		}
	}
	...
	*/
	function check($message = NULL, $ret = FALSE)
	{
		...
		{ 
			// does the user exist ?
			if(!$this->obj->user_group_model->userExists($this->obj->session->userdata('username')))
			{
				log_message('debug', "Userauth: check: deleted user still had a cookie");
				$this->logout();
			}
			...
			{
				// logout only when no rememberMe active
				if(!$this->obj->remember_me->checkRememberMe())
				{
		...
		if ($this->obj->config->item('ua_multi_language')) {
				log_message('debug', "Userauth: check: is multi language");
		...
		if ( ! $this->obj->session->userdata('loggedin') ) {
			...
			log_message('debug', "Userauth: check: not logged in checkRememberMe result  username : " .$username);
			if ( $username != FALSE ) { 
				$this->obj->user_group_model->dateStampLogin($username);
