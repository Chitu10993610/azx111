...
/**
 * 26.2.2007 tomcode
 * function permitted() :  added missing else statements 
 * function isRole() :  added missing this for class variable calls
 */
...
class Authorize {
	...
	function isRole($role = 'guest')
	{
		...
		if ( $this->obj->userauth->loggedin() ) {
			if ( ! isset( $this->is_role[$role] )) {

				// reset ACL
				$this->allowed_set = $this->denied_set = $this->allow = FALSE;
				$this->allowed_users = '';
				$this->denied_users = '';
				...
				$this->is_role[$role] = $this->permitted( $username );
			}
			log_message('debug','Authorize:isRole: '.$role.' = '.$this->is_role[$role]);
	...
	function permitted($username) {
		...
		*/
		...
		// User has set allowed list: YES
			...
			if(in_array($username, $this->denied_users)){ $deny = true; } else { $deny = false; } //tomcode: added missing else statement

			// if not denied, check if allowed
			if(!$deny && in_array($username, $this->allowed_users)){ $allow = true; } else { $allow = false; } //tomcode: added missing else statement
		}
