...
class News_cat extends Controller {
   ...
   */
   ...
   function sort() {
		...
		$affectedRows = $this->nny_news_catmodel->sortCat($aryCatOrder, $aryCatId);
   ...
   // //////////////////////////////////////////////////////////////////////////
   function browse() {
   		$this->user_group_model->can_access(VIEW_LIST_CATEGORY, null, null);
      $start = $this->uri->segment(3,0);
      $limit_per_page = 20;
//      $catId = $this->uri->segment(3,0);
	
      $this->load->model('nny_news_catmodel');                  // Instantiate the model
      
		//list all catergories
		$aryNewsTypesList = array();
		$this->nny_news_catmodel->getCategoryTree($aryNewsTypesList);
		$the_results['aryNewsTypesList'] = $aryNewsTypesList;
    
      $this->load->library('pagination');
      $this->load->helper('url');

      $config['total_rows']   = $this->nny_news_catmodel->table_record_count;
      $config['per_page']     = $limit_per_page;
      $config['uri_segment'] = 4;
      $config['num_links'] = 3;
      $config['base_url'] = base_url().'news_cat/browse/';
   ...
   // //////////////////////////////////////////////////////////////////////////
   function add() {
		$this->user_group_model->can_access(ADD_CATEGORY, null, null);
      ...
      if ( $submit != false ) {
         ...
         $this->nny_news_catmodel->createCat($data);

         redirect('news_cat/add', 'location');
      	}
      }
      ...
      if($parent_id) $data['aryCatInfo']['parent_id'] = $parent_id;
		...
		$this->_display('/ci_news/nny_news_catdetails', $data);
   }
   ...
   // //////////////////////////////////////////////////////////////////////////
   function modify() {
	$this->user_group_model->can_access(EDIT_CATEGORY, null, null);
      $submit = $this->input->post('Submit');
      $this->load->model('nny_news_catmodel');
      $catId = $this->uri->segment(3,0);
      ...
      if ( $submit != false ) {
         ...
         $this->nny_news_catmodel->editCat($data);
         ...
         $aryCatList = array();
		$this->nny_news_catmodel->getCategoryTree($aryCatList);
		$aryCatInfo = $this->nny_news_catmodel->getCatInfoBytId($catId);
		//print_r($aryCatList);
		//exit;
		$data['aryCatList'] = $aryCatList;
